...
function ADODB_Error_Handler($dbms, $fn, $errno, $errmsg, $p1, $p2, &$thisConnection)
{
	...
	case 'EXECUTE':
		...
		$s = "$errno|$errmsg|$fn|\"$sql\"|";		
	case 'PCONNECT':
	case 'CONNECT':
		$host = $p1;
		$database = $p2;
		//$s = "$dbms error: [$errno: $errmsg] in $fn($host, '****', '****', $database)\n";//break;
		$s = "$errno|$errmsg|$fn|$host|$database";		
	default:
		//$s = "$dbms error: [$errno: $errmsg] in $fn($p1, $p2)\n";//break;
		$s = "$errno|$errmsg|$fn|$p1|$p2";		
	}
	...
	*/
	if (defined('ADODB_ERROR_LOG_TYPE')) {
		...
		if (defined('ADODB_ERROR_LOG_DEST'))
			error_log("$fecha|$hora|$s", ADODB_ERROR_LOG_TYPE, ADODB_ERROR_LOG_DEST);
		else
			error_log("$fecha|$hora|$s", ADODB_ERROR_LOG_TYPE);
	}
	//print "<p>$s</p>";
