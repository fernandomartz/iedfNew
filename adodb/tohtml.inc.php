...
/*variable de estilo para los encabezados de la tabla*/
$cabecera = "style='background:green; color:#FFFFFF'";
$otros=2;
/******************************************************/
GLOBAL $gSQLMaxRows,$gSQLBlockRows,$ADODB_ROUND;
	...
	$typearr = array();
	$ncols = $rs->FieldCount();
	$hdr = "<TABLE style='font-size:11px;color:green;' COLS=$ncols $ztabhtml align='center'><tr>\n\n";
	for ($i=0; $i < $ncols; $i++) {	
		...
		$hdr .= "<TH $cabecera>$fname</TH>";
	}
	/*desde aqui se van a manejar los links*/
	//$hdr .= "<TH $cabecera>link1</TH><TH $cabecera>link2</TH>";
	/***************************************/
	$hdr .= "\n</tr>";
	if ($echo) print $hdr."\n\n";
	else $html = $hdr;
	...
	while (!$rs->EOF) {
		...
		for ($i=0; $i < $ncols; $i++) {
		   ...
		   //}else{$s .= "	<TD>". str_replace("\n",'<br>',stripslashes($i)) ."</TD>\n";}
		} // for
...
function arr2html(&$arr,$ztabhtml='',$zheaderarray='')
{
	...
	$s = "<TABLE $ztabhtml style='font-size:11px;'>";//';print_r($arr);
