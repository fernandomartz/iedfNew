...
$rs0 = $db->execute("select count(*) from dmr2015dif");
...
if($cantidad < 1) {
	...
	//$sql = "select * from dmr2015fp where distrito='".$distrito."' and seccion=".$seccion;
	...
	while(!$rs2->EOF)
	{
		...
		echo "<h2>Seccion: ".  $rs2->fields[2] ."</h2>";//Seccion
		echo "<h2>Distrito: ".  $rs2->fields[0] ."</h2>";//Distrito
		$xdel = $rs2->fields[1];
		$xseccion = $rs2->fields[2];
		//$xdistrito = $rs2->fields[0];
