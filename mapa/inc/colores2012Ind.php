
     <table border="1" style="font-size:12px; background-color:#FFF; width:200px;">
     <tr style="background-color:#CCC">
       <td style="width:150px;">PARTIDO</td><td>Color</td></tr>

     <tr><td>PAN</td>
     <td style="background-color:#2E64FE;">&nbsp;&nbsp;&nbsp;</td>
     </tr>
     
     <tr>
     <td>CC2 (PRD-PT-MC)</td>
     <td style="background-color:#FFBF00;">&nbsp;&nbsp;&nbsp;</td>
     </tr>
<?php if($datos==2) { //Partido Politico ?>  
     <tr>
     <td>PRD</td>
     <td style="background-color:#FFFF00;">&nbsp;&nbsp;&nbsp;</td>
     </tr>

     <tr>
     <td>PT</td>
     <td style="background-color:#BF00FF;">&nbsp;&nbsp;&nbsp;</td>
     </tr>

     <tr>
     <td>MC</td>
     <td style="background-color:#FF8000;">&nbsp;&nbsp;&nbsp;</td>
     </tr>
<?php } ?> 

     <tr>
     <td>PRI/CC1 (PRI-PVEM)</td>
     <td style="background-color:#FF0000;">&nbsp;&nbsp;&nbsp;</td>
     </tr>

     <tr><td>PVEM</td>
     <td style="background-color:#01DF01;">&nbsp;&nbsp;&nbsp;</td>
     </tr>

     <tr><td>NA</td>
     <td style="background-color:#00BFFF;">&nbsp;&nbsp;&nbsp;</td>
     </tr>
     
     </table>
